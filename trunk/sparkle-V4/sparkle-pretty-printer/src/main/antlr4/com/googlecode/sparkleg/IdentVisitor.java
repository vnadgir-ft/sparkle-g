...
public class IdentVisitor extends SparqlParserBaseVisitor<ST> implements SparqlParserVisitor<ST> {
		...
		if (ctx.children != null) {
			for (ParseTree c : ctx.children) {
				if (c instanceof SparqlParser.BaseDeclContext) {
					prologue.add("declaration", visitBaseDecl((SparqlParser.BaseDeclContext) c));
				} else if (c instanceof SparqlParser.PrefixDeclContext) {
					prologue.add("declaration", visitPrefixDecl((SparqlParser.PrefixDeclContext) c));
				}
			}
		}
	...
	public ST visitUpdateCommand(SparqlParser.UpdateCommandContext ctx) {
		...
		if (ctx != null) {
			for (ParseTree c : ctx.children) {
				if (c instanceof SparqlParser.PrologueContext) {
					updateCommand.add("prologue", visitPrologue((SparqlParser.PrologueContext) c));
				} else if (c instanceof SparqlParser.UpdateContext) {
					updateCommand.add("update", visitUpdate((SparqlParser.UpdateContext) c));
				}
			}
		}
